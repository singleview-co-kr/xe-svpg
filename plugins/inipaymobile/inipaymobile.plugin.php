<?php
class inipaymobile extends SvpgPlugin 
{
	var $plugin_info;
 ...
 */
	function getFormData($args)
	{
		...
		$tpl_path = _XE_PATH_."modules/svpg/plugins/inipaymobile/tpl";
	...
	function processReview($args)
	{
		...
		*/
		...
		Context::set('next_url', getNotEncodedFullUrl('') . '/' . $svpg_module_info->mid . '?n_page=' . $args->order_srl);
		Context::set('return_url', getNotEncodedFullUrl('') . '/' . $svpg_module_info->mid . '?r_page=' . $args->order_srl);
		Context::set('noti_url', getNotEncodedFullUrl('','module','svpg','act','procSvpgDoPayment','module_srl',$args->module_srl, 'svpg_module_srl',$args->svpg_module_srl, 'plugin_srl',$this->plugin_info->plugin_srl, 'order_srl', $args->order_srl, 'svpg_target_module', $args->target_module));
...
//debugPrint($PGIP);
		...
		{
			$obj = new BaseObject(-1, 'msg_invalid_request');
		...
		$output = new BaseObject(0, $P_RMESG1 . ' ' . $P_RMESG2);
	...
	{
                $oTemplate = &TemplateHandler::getInstance();
                $tpl_path = _XE_PATH_."modules/svpg3/plugins/inipaymobile/tpl";
...
/* Location: ./modules/svpg/plugins/inipaymobile/inipaymobile.plugin.php */